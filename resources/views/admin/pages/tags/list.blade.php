...
@section('topbar-text', 'Tag Management')
...
<div class="container-xxl tag-container">
    ...
    <div class="row stats-row g-3">
        <div class="col-xl-6 col-md-6">
            <div class="stat-card">
                @php
                    $totalTags = DB::table('tags')->where('is_deleted', 0)->count();
                @endphp
        ...
        <div class="col-xl-6 col-md-6">
            <div class="stat-card green">
                @php
                    $totalProducts = DB::table('product_tags')
                        ->distinct('product_id')
                        ->count('product_id');
                @endphp
    ...
    <div class="filter-section">
        ...
        <form id="filterForm" method="GET" action="{{ route('tag.index') }}">
            <div class="filter-row">
                <div class="filter-group">
                    <label class="filter-label">Search Tag</label>
                    <input type="text" name="search" class="filter-input" placeholder="Search by tag name..." 
    ...
    <div class="tag-card">
        <div class="card-header-custom">
            ...
            <a href="{{ route('tag.create') }}" class="btn btn-add-tag">
                <iconify-icon icon="solar:add-circle-bold" class="me-2"></iconify-icon>
                Add Tag
            </a>
        </div>
        
        <div class="table-container" id="tagTableContainer">
            @include('admin.pages.tags.partials.tag-table')
            
            <!-- Pagination -->
            @include('admin.pages.tags.partials.pagination')
        </div>
    </div>
...
@section('admin-js')
<script>
    var deleteUrl = "{{ route('tag.destroy', ['id' => ':id']) }}";
    
    $(document).ready(function() {
        // Function to load tags with AJAX
        function loadTags(page = 1) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#tagTableContainer').html(response.table + response.pagination);
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                Swal.fire({
                                    title: "Deleted!",
                                    text: response.message || "Tag has been deleted.",
                                    icon: "success",
