...
@section('topbar-text', 'Size Management')
...
<div class="container-xxl size-container">
    ...
    <div class="row stats-row g-3">
        <div class="col-xl-4 col-md-6">
            <div class="stat-card">
                @php
                    $totalSizes = DB::table('sizes')->where('is_deleted', 0)->count();
                @endphp
        ...
        <div class="col-xl-4 col-md-6">
            <div class="stat-card blue">
                @php
                    $activeSizes = DB::table('sizes')
                        ->where('is_deleted', 0)
                        ->whereExists(function($query) {
                            $query->select(DB::raw(1))
                                  ->from('product_attributes')
                                  ->whereColumn('product_attributes.size_id', 'sizes.id');
                        })
                        ->count();
                @endphp
                <div class="stat-value">{{ $activeSizes }}</div>
                <div class="stat-label">Active Sizes</div>
            </div>
        </div>
        <div class="col-xl-4 col-md-6">
            <div class="stat-card green">
                ...
                <div class="stat-label">Products with Sizes</div>
            </div>
        </div>
    </div>
    ...
    <div class="filter-section">
        ...
        <form id="filterForm" method="GET" action="{{ route('size.index') }}">
            <div class="filter-row">
                <div class="filter-group">
                    <label class="filter-label">Search Size</label>
                    <input type="text" name="search" class="filter-input" placeholder="Search by size name..." 
    ...
    <div class="size-card">
        <div class="card-header-custom">
            ...
            <a href="{{ route('size.create') }}" class="btn btn-add-size">
                <iconify-icon icon="solar:add-circle-bold" class="me-2"></iconify-icon>
                Add Size
            </a>
        </div>
        
        <div class="table-container" id="sizeTableContainer">
            @include('admin.pages.size.partials.size-table')
            
            <!-- Pagination -->
            @include('admin.pages.size.partials.pagination')
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function() {
        // Function to load sizes with AJAX
        function loadSizes(page = 1) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#sizeTableContainer').html(response.table + response.pagination);
        ...
        function bindDeleteButtons() {
            $('.delete-btn').off('click').on('click', function() {
                ...
                Swal.fire({
                    title: "Are you sure?",
                    text: "You won't be able to recover this size!",
                    icon: "warning",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                Swal.fire({
                                    title: "Deleted!",
                                    text: response.message || "Size has been deleted.",
                                    icon: "success",
                            ...
                            error: function(response) {
                                Swal.fire({
                                    title: "Error!",
                                    text: "An error occurred while deleting the size.",
                                    icon: "error",
                                    confirmButtonColor: '#e74c3c'
                                });
                            }
